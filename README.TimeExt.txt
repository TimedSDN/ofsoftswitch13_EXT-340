...
====================
The current version supports multiple concurrent Scheduled Bundles; every switch can have up to 5 concurrent pending scheduled bundles.
Important notes: in this version Scheduled Bundles must use bundle IDs in the range [0..4]. The controller (Dpctl) can schedule up to 5 bundles per switch using different bundle IDs from this range. Once a Scheduled Bundle has been committed, the controller can reuse the corresponding bundle ID.
...
After completing these three steps, Dpctl waits until the switch completes the scheduled bundle and sends an acknowledgement. This may take a while. It is thus recommended to run the commit line in the background (using an '&' at the end of the line).
